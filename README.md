...
**事实上，不得不承认，本程序有很多地方写的像屎山（比如不能自定义名字，不能选择分析的月份等等，这些地方需要用户自行更改代码），但是我实在是太懒太懒了，而且此程序也只不过是本人（在读大二学生）利用三天课余时间给女朋友准备的惊喜罢了。。。此后碍于课程压力或许很难再更新此程序。。。故烦请大佬嘴下留情，毕竟对于我来说此程序已经实现功能和价值。如果有人想改进此程序，欢迎随时clone+push。我会不定期查看。谢谢大家。**
...
注：如果不想要情感分析在软件提示输入APIID的时候退出即可，不影响其他内容的分析。如果要使用情感分析功能，则需要自行去百度智能云获取api，此处不再赘述。
